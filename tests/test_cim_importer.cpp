...
{
    return result.summary().diagnosticText();
}
...
void TestCimImporter::importer_creates_minimal_model_for_directory()
{
    ...
    QCOMPARE(summary.objectCount, result.model.objectCount());
    QCOMPARE(summary.profileCount, result.loadedProfiles.size());
    QCOMPARE(summary.errorCount, 0);
    QCOMPARE(summary.warningCount, 0);
    QVERIFY(!summary.hasErrors());
    QVERIFY(!summary.hasWarnings());
    QCOMPARE(summary.loadedProfiles, result.loadedProfiles);
    QCOMPARE(summary.classDistribution.value(QStringLiteral("Breaker")), 1);
    QVERIFY(summary.diagnosticText().contains(QStringLiteral("success=1")));
    QVERIFY(summary.diagnosticText().contains(QStringLiteral("count=6")));
...
void TestCimImporter::graphic_mapper_maps_core_classes()
{
    ...
    QCOMPARE(mapper.drawTypeForObject(breaker), CIMDRAW_SLD_CIRCUIT_BREAKER);
    QCOMPARE(mapper.drawTypeForObject(disconnector), CIMDRAW_SLD_DISCONNECTOR);
    QCOMPARE(mapper.drawTypeForObject(busbar), CIMDRAW_SLD_BUSBAR_SECTION);
    QCOMPARE(mapper.drawTypeForObject(transformer), CIMDRAW_SLD_TRANSFORMER_2W);
    QCOMPARE(mapper.drawTypeForObject(autoTransformer), CIMDRAW_WSYM_AUTO_XFMR);
    QCOMPARE(mapper.drawTypeForObject(stationTransformer), CIMDRAW_WSYM_STATION_XFMR);
    QCOMPARE(mapper.drawTypeForObject(load), CIMDRAW_SLD_LOAD);
    QCOMPARE(mapper.drawTypeForObject(generator), CIMDRAW_SLD_GENERATOR);

    const CimGraphicMappingResult breakerMapping = mapper.mapObject(breaker);
    QVERIFY(breakerMapping.isMappable());
    QCOMPARE(breakerMapping.mrid, breaker.mrid);
    QCOMPARE(breakerMapping.className, breaker.className);
    QCOMPARE(breakerMapping.drawType, CIMDRAW_SLD_CIRCUIT_BREAKER);
    QCOMPARE(static_cast<int>(breakerMapping.role),
             static_cast<int>(CimGraphicMappingRole::SwitchingDevice));
    QVERIFY(!breakerMapping.requiresDedicatedCapacitorItemFactory);

    const CimGraphicMappingResult unsupportedMapping = mapper.mapObject(unsupported);
    QVERIFY(!unsupportedMapping.isMappable());
    QCOMPARE(unsupportedMapping.drawType, -1);
    QCOMPARE(static_cast<int>(unsupportedMapping.role),
             static_cast<int>(CimGraphicMappingRole::Unsupported));
...
void TestCimImporter::graphic_mapper_maps_p0_gap_classes()
{
    ...
    QCOMPARE(mapper.drawTypeForObject(line), CIMDRAW_WSYM_CABLE);
    QCOMPARE(mapper.drawTypeForObject(loadBreak), CIMDRAW_WSYM_LOAD_BREAK);
    QCOMPARE(mapper.drawTypeForObject(groundDisconnector), CIMDRAW_WSYM_EARTH_SWITCH);
    QCOMPARE(mapper.drawTypeForObject(shunt), CIMDRAW_WSYM_CAPACITOR);
...
void TestCimImporter::graphic_mapper_maps_p1_reuse_classes()
{
    ...
    QCOMPARE(mapper.drawTypeForObject(shunt), CIMDRAW_WSYM_CAPACITOR);
    QCOMPARE(mapper.drawTypeForObject(series), CIMDRAW_WSYM_REACTOR);
    QCOMPARE(mapper.drawTypeForObject(asyncMachine), CIMDRAW_SLD_MOTOR);
    QCOMPARE(mapper.drawTypeForObject(externalInjection), CIMDRAW_WSYM_GRID);
    QCOMPARE(mapper.drawTypeForObject(equivalentInjection), CIMDRAW_WSYM_GRID);
    QCOMPARE(mapper.drawTypeForObject(arrester), CIMDRAW_WSYM_ARRESTER);
    QCOMPARE(mapper.drawTypeForObject(ground), CIMDRAW_SLD_GROUND);
...
void TestCimImporter::graphic_mapper_maps_p2_missing_graphics()
{
    ...
    QCOMPARE(mapper.drawTypeForObject(currentTransformer), CIMDRAW_WSYM_CURRENT_TRANSFORMER);
    QCOMPARE(mapper.drawTypeForObject(potentialTransformer), CIMDRAW_WSYM_VOLTAGE_TRANSFORMER);
    QCOMPARE(mapper.drawTypeForObject(voltageTransformer), CIMDRAW_WSYM_VOLTAGE_TRANSFORMER);
    QCOMPARE(mapper.drawTypeForObject(fuse), CIMDRAW_WSYM_FUSE);
...
void TestCimImporter::graphic_mapper_maps_projection_infeed_classes()
{
    ...
    QCOMPARE(mapper.drawTypeForObject(solar), CIMDRAW_WSYM_PV_INFEED);
    QCOMPARE(mapper.drawTypeForObject(wind), CIMDRAW_WSYM_WIND_INFEED);
    QCOMPARE(mapper.drawTypeForObject(battery), CIMDRAW_WSYM_ESS);
...
void TestCimImporter::graphic_mapper_maps_transformer_3w_when_three_ends_exist()
{
    ...
    QCOMPARE(mapper.drawTypeForObject(transformer2w), CIMDRAW_SLD_TRANSFORMER_2W);
    QCOMPARE(mapper.drawTypeForObject(transformer3w), CIMDRAW_WSYM_TRANSFORMER_3W);
...
void TestCimImporter::graphic_mapper_refines_power_transformer_specializations_from_name_tokens()
{
    ...
    QCOMPARE(mapper.drawTypeForObject(autoTransformer), CIMDRAW_WSYM_AUTO_XFMR);
    QCOMPARE(mapper.drawTypeForObject(earthingTransformer), CIMDRAW_WSYM_EARTHING_XFMR);
    QCOMPARE(mapper.drawTypeForObject(stationTransformer), CIMDRAW_WSYM_STATION_XFMR);
    QCOMPARE(mapper.drawTypeForObject(plainTransformer), CIMDRAW_SLD_TRANSFORMER_2W);
    QCOMPARE(mapper.drawTypeForObject(autoTransformerByAttribute), CIMDRAW_WSYM_AUTO_XFMR);
    QCOMPARE(mapper.drawTypeForObject(stationTransformerByAttribute), CIMDRAW_WSYM_STATION_XFMR);
}
...
void TestCimImporter::graphic_mapper_refines_shunt_compensator_family_from_parameters_and_classes()
{
    ...
    QCOMPARE(mapper.drawTypeForObject(capacitor), CIMDRAW_WSYM_CAPACITOR);
    QCOMPARE(mapper.drawTypeForObject(reactor), CIMDRAW_WSYM_REACTOR);
    QCOMPARE(mapper.drawTypeForObject(splitReactor), CIMDRAW_WSYM_SPLIT_REACTOR);
    QCOMPARE(mapper.drawTypeForObject(nonlinearReactor), CIMDRAW_WSYM_REACTOR);
    QCOMPARE(mapper.drawTypeForObject(svg), CIMDRAW_WSYM_SVG_COMP);
    QCOMPARE(mapper.drawTypeForObject(arcCoil), CIMDRAW_WSYM_ARC_COIL);

    const CimGraphicMappingResult capacitorMapping = mapper.mapObject(capacitor);
    QCOMPARE(capacitorMapping.drawType, CIMDRAW_WSYM_CAPACITOR);
    QCOMPARE(static_cast<int>(capacitorMapping.role),
             static_cast<int>(CimGraphicMappingRole::ShuntCompensator));
    QVERIFY(capacitorMapping.requiresDedicatedCapacitorItemFactory);

    const CimGraphicMappingResult svgMapping = mapper.mapObject(svg);
    QCOMPARE(svgMapping.drawType, CIMDRAW_WSYM_SVG_COMP);
    QCOMPARE(static_cast<int>(svgMapping.role),
             static_cast<int>(CimGraphicMappingRole::ShuntCompensator));
    QVERIFY(!svgMapping.requiresDedicatedCapacitorItemFactory);
}
...
void TestCimImporter::graphic_mapper_refines_series_compensator_family_from_name_and_reactance()
{
    ...
    QCOMPARE(mapper.drawTypeForObject(seriesCapacitor), CIMDRAW_WSYM_CAPACITOR);
    QCOMPARE(mapper.drawTypeForObject(seriesReactor), CIMDRAW_WSYM_REACTOR);
    QCOMPARE(mapper.drawTypeForObject(reactanceCapacitor), CIMDRAW_WSYM_CAPACITOR);
    QCOMPARE(mapper.drawTypeForObject(reactanceReactor), CIMDRAW_WSYM_REACTOR);

    const CimGraphicMappingResult seriesCapacitorMapping = mapper.mapObject(seriesCapacitor);
    QCOMPARE(seriesCapacitorMapping.drawType, CIMDRAW_WSYM_CAPACITOR);
    QCOMPARE(static_cast<int>(seriesCapacitorMapping.role),
             static_cast<int>(CimGraphicMappingRole::SeriesCompensator));
    QVERIFY(seriesCapacitorMapping.requiresDedicatedCapacitorItemFactory);
}
...
void TestCimImporter::id_index_binds_bidirectionally()
{
    ...
    QCOMPARE(index.mrids(), QStringList{QStringLiteral("mrid-001")});
}

void TestCimImporter::topology_mapper_builds_minimal_snapshot_from_cim_model()
{
    CimModel model;

    CimObject busbar;
    busbar.mrid = QStringLiteral("busbar-001");
    busbar.className = QStringLiteral("BusbarSection");
    ...
    breaker.setAttribute(QStringLiteral("Switch.open"), QStringLiteral("false"));
    QVERIFY(model.addObject(breaker));

    CimObject load;
    load.mrid = QStringLiteral("load-001");
    load.className = QStringLiteral("EnergyConsumer");
    load.name = QStringLiteral("L1");
    QVERIFY(model.addObject(load));

    CimObject connectivityNode;
    connectivityNode.mrid = QStringLiteral("cn-load-001");
    connectivityNode.className = QStringLiteral("ConnectivityNode");
    connectivityNode.addReference(QStringLiteral("ConnectivityNode.TopologicalNode"), QStringLiteral("tn-load-001"));
    QVERIFY(model.addObject(connectivityNode));

    auto addTerminal = [&](const QString& terminalMrid,
                           const QString& equipmentMrid,
                           const QString& topologicalNodeMrid,
                           const QString& connectivityNodeMrid,
                           int sequenceNumber) {
        CimObject terminal;
        ...
        if (!topologicalNodeMrid.isEmpty())
            terminal.addReference(QStringLiteral("Terminal.TopologicalNode"), topologicalNodeMrid);
        if (!connectivityNodeMrid.isEmpty())
            terminal.addReference(QStringLiteral("Terminal.ConnectivityNode"), connectivityNodeMrid);
        if (sequenceNumber > 0)
            terminal.setAttribute(QStringLiteral("ACDCTerminal.sequenceNumber"), sequenceNumber);
        QVERIFY(model.addObject(terminal));
    };

    addTerminal(QStringLiteral("terminal-busbar-001"),
                QStringLiteral("busbar-001"),
                QStringLiteral("tn-bus-001"),
                QString(),
                1);
    addTerminal(QStringLiteral("terminal-breaker-a"),
                QStringLiteral("breaker-001"),
                QStringLiteral("tn-bus-001"),
                QString(),
                1);
    addTerminal(QStringLiteral("terminal-breaker-b"),
                QStringLiteral("breaker-001"),
                QStringLiteral("tn-load-001"),
                QString(),
                2);
    addTerminal(QStringLiteral("terminal-load-001"),
                QStringLiteral("load-001"),
                QString(),
                QStringLiteral("cn-load-001"),
                1);

    CimTopologyMapper mapper;
    const PowerTopologyAnalysisSnapshot snapshot = mapper.build(model);

    QCOMPARE(snapshot.devices.size(), 3);
    QCOMPARE(snapshot.conductors.size(), 0);
    QCOMPARE(snapshot.nodes.size(), 2);
    QVERIFY(snapshot.deviceIndex.contains(QStringLiteral("busbar-001")));
    QVERIFY(snapshot.deviceIndex.contains(QStringLiteral("breaker-001")));
    QVERIFY(snapshot.deviceIndex.contains(QStringLiteral("load-001")));

    const PowerTopologyDeviceRecord& breakerRecord =
        snapshot.devices.at(snapshot.deviceIndex.value(QStringLiteral("breaker-001")));
    QCOMPARE(breakerRecord.role, PowerTopologyRole::Breaker);
    QCOMPARE(breakerRecord.terminalIds.size(), 2);
    QCOMPARE(breakerRecord.terminalIds.at(0), QStringLiteral("terminal-breaker-a"));
    QCOMPARE(breakerRecord.terminalIds.at(1), QStringLiteral("terminal-breaker-b"));
    QCOMPARE(breakerRecord.internalConnectivity.value(
                 QStringLiteral("terminal-breaker-a|terminal-breaker-b")),
             true);

    QVERIFY(snapshot.terminalToNodeIndex.contains(QStringLiteral("terminal-breaker-a")));
    QVERIFY(snapshot.terminalToNodeIndex.contains(QStringLiteral("terminal-breaker-b")));
    QVERIFY(snapshot.terminalToNodeIndex.contains(QStringLiteral("terminal-load-001")));

    const int busNodeId = snapshot.terminalToNodeIndex.value(QStringLiteral("terminal-breaker-a"));
    const int loadNodeId = snapshot.terminalToNodeIndex.value(QStringLiteral("terminal-breaker-b"));
    QCOMPARE(snapshot.terminalToNodeIndex.value(QStringLiteral("terminal-busbar-001")), busNodeId);
    QCOMPARE(snapshot.terminalToNodeIndex.value(QStringLiteral("terminal-load-001")), loadNodeId);
    QVERIFY(snapshot.nodeIdToDeviceIds.value(busNodeId).contains(QStringLiteral("busbar-001")));
    QVERIFY(snapshot.nodeIdToDeviceIds.value(busNodeId).contains(QStringLiteral("breaker-001")));
    QVERIFY(snapshot.nodeIdToDeviceIds.value(loadNodeId).contains(QStringLiteral("breaker-001")));
    QVERIFY(snapshot.nodeIdToDeviceIds.value(loadNodeId).contains(QStringLiteral("load-001")));
}

void TestCimImporter::importer_exposes_model_topology_snapshot()
{
    QTemporaryDir dir;
    QVERIFY(dir.isValid());
...
<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
         xmlns:cim="http://iec.ch/TC57/2013/CIM-schema-cim16#"
         xmlns:md="http://iec.ch/TC57/61970-552/ModelDescription/1#">
  <md:FullModel rdf:about="urn:uuid:importer-topology-eq">
    <md:Model.profile>http://iec.ch/TC57/ns/CIM/Equipment-EU/3.0</md:Model.profile>
  </md:FullModel>
  <cim:BusbarSection rdf:ID="busbar-001">
    <cim:IdentifiedObject.name>BB1</cim:IdentifiedObject.name>
  </cim:BusbarSection>
  <cim:Breaker rdf:ID="breaker-001">
    <cim:IdentifiedObject.name>QF1</cim:IdentifiedObject.name>
    <cim:Switch.open>false</cim:Switch.open>
  </cim:Breaker>
  <cim:EnergyConsumer rdf:ID="load-001">
    <cim:IdentifiedObject.name>L1</cim:IdentifiedObject.name>
  </cim:EnergyConsumer>
  <cim:Terminal rdf:ID="terminal-busbar-001">
    <cim:Terminal.ConductingEquipment rdf:resource="#busbar-001"/>
    <cim:Terminal.TopologicalNode rdf:resource="#tn-bus-001"/>
    <cim:ACDCTerminal.sequenceNumber>1</cim:ACDCTerminal.sequenceNumber>
  </cim:Terminal>
  <cim:Terminal rdf:ID="terminal-breaker-a">
    <cim:Terminal.ConductingEquipment rdf:resource="#breaker-001"/>
    <cim:Terminal.TopologicalNode rdf:resource="#tn-bus-001"/>
    <cim:ACDCTerminal.sequenceNumber>1</cim:ACDCTerminal.sequenceNumber>
  </cim:Terminal>
  <cim:Terminal rdf:ID="terminal-breaker-b">
    <cim:Terminal.ConductingEquipment rdf:resource="#breaker-001"/>
    <cim:Terminal.TopologicalNode rdf:resource="#tn-load-001"/>
    <cim:ACDCTerminal.sequenceNumber>2</cim:ACDCTerminal.sequenceNumber>
  </cim:Terminal>
  <cim:Terminal rdf:ID="terminal-load-001">
    <cim:Terminal.ConductingEquipment rdf:resource="#load-001"/>
    <cim:Terminal.TopologicalNode rdf:resource="#tn-load-001"/>
    <cim:ACDCTerminal.sequenceNumber>1</cim:ACDCTerminal.sequenceNumber>
  </cim:Terminal>
</rdf:RDF>)");

    QVERIFY2(writeUtf8File(dir.filePath(QStringLiteral("importer_topology_EQ.xml")), eqXml),
             "failed to write importer topology EQ sample");
    ...
    QVERIFY2(result.success, qPrintable(importDiagnostics(result)));
    QVERIFY(result.topologySnapshot.devices.size() > 0);
    QVERIFY(result.topologySnapshot.nodes.size() > 0);
    QVERIFY(result.topologySnapshot.deviceIndex.contains(QStringLiteral("breaker-001")));
    QVERIFY(result.topologySnapshot.deviceIndex.contains(QStringLiteral("load-001")));
    QVERIFY(result.topologySnapshot.terminalToNodeIndex.size() > 0);
}

void TestCimImporter::importer_topology_projection_uses_unified_query_entry()
{
    QTemporaryDir dir;
    QVERIFY(dir.isValid());
...
<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
         xmlns:cim="http://iec.ch/TC57/2013/CIM-schema-cim16#"
         xmlns:md="http://iec.ch/TC57/61970-552/ModelDescription/1#">
  <md:FullModel rdf:about="urn:uuid:importer-topology-projection-eq">
    <md:Model.profile>http://iec.ch/TC57/ns/CIM/Equipment-EU/3.0</md:Model.profile>
  </md:FullModel>
  <cim:ExternalNetworkInjection rdf:ID="grid-001">
  ...
  <cim:Ground rdf:ID="ground-001">
    <cim:IdentifiedObject.name>GND1</cim:IdentifiedObject.name>
  </cim:Ground>
  <cim:Terminal rdf:ID="terminal-grid-001">
    <cim:Terminal.ConductingEquipment rdf:resource="#grid-001"/>
    <cim:Terminal.TopologicalNode rdf:resource="#tn-source"/>
    <cim:ACDCTerminal.sequenceNumber>1</cim:ACDCTerminal.sequenceNumber>
  </cim:Terminal>
  <cim:Terminal rdf:ID="terminal-breaker-a">
    <cim:Terminal.ConductingEquipment rdf:resource="#breaker-001"/>
  ...
  <cim:Terminal rdf:ID="terminal-breaker-b">
    <cim:Terminal.ConductingEquipment rdf:resource="#breaker-001"/>
    <cim:Terminal.TopologicalNode rdf:resource="#tn-load"/>
    <cim:ACDCTerminal.sequenceNumber>2</cim:ACDCTerminal.sequenceNumber>
  </cim:Terminal>
  <cim:Terminal rdf:ID="terminal-load-001">
    <cim:Terminal.ConductingEquipment rdf:resource="#load-001"/>
  ...
  <cim:Terminal rdf:ID="terminal-ground-001">
    <cim:Terminal.ConductingEquipment rdf:resource="#ground-001"/>
    <cim:Terminal.TopologicalNode rdf:resource="#tn-load"/>
    <cim:ACDCTerminal.sequenceNumber>1</cim:ACDCTerminal.sequenceNumber>
  </cim:Terminal>
</rdf:RDF>)");

    QVERIFY2(writeUtf8File(dir.filePath(QStringLiteral("importer_topology_projection_EQ.xml")), eqXml),
             "failed to write importer topology projection EQ sample");

    CgmesPackageImporter importer;
    const CimImportResult result = importer.importFromPath(dir.path());

    QVERIFY2(result.success, qPrintable(importDiagnostics(result)));
    QCOMPARE(result.topologyProjection.sourceKind(), PowerTopologyProjectionSourceKind::ImportedModel);
    QCOMPARE(result.topologyProjection.sourceKindKey(), QStringLiteral("importedModel"));
    QVERIFY(result.topologyProjection.truthBoundaryText().contains(QStringLiteral("导入数据真源")));

    const QStringList reachable = result.topologyProjection.reachableDevices(QStringLiteral("breaker-001"));
    ...
    const PowerTopologyPath supplyPath =
        result.topologyProjection.shortestSupplyPath(QStringLiteral("grid-001"), QStringLiteral("load-001"));
    QVERIFY(supplyPath.found);
    QCOMPARE(supplyPath.deviceIds, QStringList({QStringLiteral("grid-001"),
                                                QStringLiteral("breaker-001"),
                                                QStringLiteral("load-001")}));

    const PowerTopologyGroundPath groundPath =
        result.topologyProjection.shortestGroundPath(QStringLiteral("grid-001"));
    QVERIFY(groundPath.found);
    QCOMPARE(groundPath.deviceIds, QStringList({QStringLiteral("grid-001"),
                                                QStringLiteral("breaker-001"),
