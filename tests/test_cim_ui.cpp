...
void TestCimUi::import_creates_browser_dock_and_scene_items()
{
    ...
    LzScene* scene = frame.activeScene();
...
void TestCimUi::import_layout_spreads_primary_equipment_positions()
{
    ...
    for (QGraphicsItem* item : sceneItems)
    {
        ...
        if (shape->lzObjectId() == QStringLiteral("busbar-001"))
            busbarShape = shape;
        else if (shape->lzObjectId() == QStringLiteral("breaker-001"))
            breakerShape = shape;
        else if (shape->lzObjectId() == QStringLiteral("load-001"))
            loadShape = shape;
    }
...
void TestCimUi::import_generates_readable_connection_lines()
{
    ...
    for (QGraphicsItem* item : sceneItems)
    {
        ...
        auto* shape = dynamic_cast<TmpShape*>(item);
        if (shape && shape->lzObjectId() == QStringLiteral("busbar-001"))
            busbarShape = shape;
    }
...
void TestCimUi::browser_selection_locates_scene_item()
{
    ...
    QCOMPARE(selectedShape->lzObjectId(), QStringLiteral("breaker-001"));
}
