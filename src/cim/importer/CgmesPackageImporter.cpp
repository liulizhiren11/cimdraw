...
void CgmesPackageImporter::synthesizeInverseTransformerEndReferences(QHash<QString, ParsedObjectData>& objectMap)
{
    ...
    for (auto it = objectMap.constBegin(); it != objectMap.constEnd(); ++it)
    {
        ...
        const QString transformerMrid = transformerEndView.powerTransformerMrid();
        if (transformerMrid.isEmpty())
            continue;

        auto transformerIt = objectMap.find(transformerMrid);
...
CimImportResult CgmesPackageImporter::importFromPath(const QString& path) const
{
    ...
    auto finalizeResult = [&]() {
        ...
        result.topologyProjection = PowerTopologyProjection::fromImportedModel(result.model);
        result.topologySnapshot = result.topologyProjection.snapshot();
