...
QString firstReferenceTarget(const CimObject& object, const QString& relationName)
{
...
LzConnectPoint* preferredWiringPort(TmpShape* shape, CONNECT_DIRECTION direction)
{
    if (!shape)
        return nullptr;
...
QSizeF preferredSceneItemSizeForDrawType(LzDrawTypeId drawType, const QSizeF& fallback)
{
    switch (drawType)
    {
...
int optionalIntAttribute(const CimObject& object, std::initializer_list<const char*> keys)
{
    for (const char* key : keys)
    {
        bool ok = false;
        const int value = object.attribute(QString::fromLatin1(key)).toInt(&ok);
        if (ok)
            return value;
    }
    return -1;
...
{
    const QString transformerEndMrid = firstReferenceTarget(terminal, QStringLiteral("Terminal.TransformerEnd"));
    ...
    for (auto it = objectByMrid.constBegin(); it != objectByMrid.constEnd(); ++it)
    {
        ...
        if (!object || object->className != QStringLiteral("PowerTransformerEnd"))
            continue;
        if (firstReferenceTarget(*object, QStringLiteral("TransformerEnd.Terminal")) == terminal.mrid)
            return object;
    }
    return nullptr;
}
...
{
    if (identifiedObject.className == QStringLiteral("Terminal"))
        return firstReferenceTarget(identifiedObject, QStringLiteral("Terminal.ConductingEquipment"));
    return identifiedObject.mrid;
}

bool orderedTransformerShape(TmpShape* shape)
{
    return dynamic_cast<LzPowerTransformer2wItem*>(shape) != nullptr
        || dynamic_cast<LzPowerAutoXfmrItem*>(shape) != nullptr
        || dynamic_cast<LzPowerEarthingXfmrItem*>(shape) != nullptr
        || dynamic_cast<LzPowerStationXfmrItem*>(shape) != nullptr
        || dynamic_cast<LzPowerTransformer3wItem*>(shape) != nullptr;
}
...
QHash<QString, CimDiagramGeometry> buildDiagramGeometryByEquipmentMrid(const QVector<CimObject>& objects)
{
    ...
    for (const CimObject& object : objects)
    {
        if (object.className == QStringLiteral("DiagramObject"))
        {
            const QString identifiedMrid =
                firstReferenceTarget(object, QStringLiteral("DiagramObject.IdentifiedObject"));
            ...
            const QVariant rotationValue = object.attribute(QStringLiteral("DiagramObject.rotation"));
            bool ok = false;
            const qreal rotation = rotationValue.toDouble(&ok);
            if (ok)
                rotationByDiagramObjectMrid.insert(object.mrid, rotation);
            continue;
        }

        if (object.className != QStringLiteral("DiagramObjectPoint"))
            continue;

        const QString diagramObjectMrid =
            firstReferenceTarget(object, QStringLiteral("DiagramObjectPoint.DiagramObject"));
        if (diagramObjectMrid.isEmpty())
            continue;

        bool xOk = false;
        bool yOk = false;
        const qreal x = object.attribute(QStringLiteral("DiagramObjectPoint.xPosition")).toDouble(&xOk);
        const qreal y = object.attribute(QStringLiteral("DiagramObjectPoint.yPosition")).toDouble(&yOk);
        if (!xOk || !yOk)
            continue;

        bool sequenceOk = false;
        const int sequence =
            object.attribute(QStringLiteral("DiagramObjectPoint.sequenceNumber")).toInt(&sequenceOk);

        CimDiagramPoint point;
        point.sequence = sequenceOk ? sequence : pointsByDiagramObjectMrid.value(diagramObjectMrid).size() + 1;
...
CimSceneBuilder::BuildResult CimSceneBuilder::populateSceneFromCimModel(LzScene* scene, const CimModel& model) const
{
    BuildResult result;
    if (!scene)
        return result;
    ...
    for (const CimObject& object : objects)
    {
        ...
        if (mapper.canMap(object))
        {
            ...
            else if (cimSwitchClass(object.className))
                switchingDevices.push_back(object);
            else if (object.className == QStringLiteral("PowerTransformer"))
                transformers.push_back(object);
            else if (cimSourceClass(object.className))
                sources.push_back(object);
            else if (cimLoadClass(object.className))
                loads.push_back(object);
            else if (cimMotorClass(object.className))
                motors.push_back(object);
            else if (cimMeasurementClass(object.className))
                measurements.push_back(object);
            else if (cimFuseClass(object.className))
                fuses.push_back(object);
            else if (cimJunctionClass(object.className))
    ...
    auto createGraphicItem = [&](const CimObject& object, const QPointF& topLeft, const QSizeF& size) {
        const LzDrawTypeId drawType = mapper.drawTypeForObject(object);
        ...
        if (drawType == LZ_WSYM_CAPACITOR
            && (cimCapacitorLikeShuntClass(object.className)
                || object.className == QStringLiteral("SeriesCompensator")))
        {
            // `LZ_WSYM_CAPACITOR` shares historical id `21` with `LZ_SLD_GENERATOR`,
            // so CIM capacitors must bypass drawType-only tool lookup here.
            auto* capacitor = new LzPowerCapacitorItem(rect);
            scene->addItem(capacitor);
            graphicsItem = capacitor;
        }
        else
        {
            LzTool* tool = LzToolManager::getInstance()->changeTool(drawType);
            if (!tool)
        ...
        shape->setLzObjectId(object.mrid);
        ...
        if (auto* wiring = dynamic_cast<LzWiringItemBase*>(shape))
        {
            ...
            {
                const bool isOpen = object.attribute(QStringLiteral("Switch.open")).toString()
                                        .compare(QStringLiteral("true"), Qt::CaseInsensitive) == 0;
                wiring->setSwitchPosition(isOpen ? 0 : 1);
            }
        }
    ...
    for (const CimObject& object : others)
    {
        if (object.className == QStringLiteral("ACLineSegment"))
            conductors.push_back(object);
        else if (cimShuntCompensationClass(object.className))
            shunts.push_back(object);
        else if (object.className == QStringLiteral("SeriesCompensator"))
            seriesCompensators.push_back(object);
        else if (object.className == QStringLiteral("SurgeArrester"))
    ...
    for (const CimObject& object : objects)
    {
        if (object.className != QStringLiteral("Terminal"))
            continue;

        const QString equipmentMrid = firstReferenceTarget(object, QStringLiteral("Terminal.ConductingEquipment"));
        QString nodeMrid = firstReferenceTarget(object, QStringLiteral("Terminal.TopologicalNode"));
        if (nodeMrid.isEmpty())
            nodeMrid = firstReferenceTarget(object, QStringLiteral("Terminal.ConnectivityNode"));
        if (nodeMrid.isEmpty())
        {
            const QString connectivityNodeMrid = firstReferenceTarget(object, QStringLiteral("Terminal.ConnectivityNode"));
            if (const CimObject* connectivityNode = objectByMrid.value(connectivityNodeMrid, nullptr))
                nodeMrid = firstReferenceTarget(*connectivityNode, QStringLiteral("ConnectivityNode.TopologicalNode"));
        }
        ...
        binding.terminalSequence = optionalIntAttribute(object,
        ...
        {
            binding.transformerEndNumber = optionalIntAttribute(*transformerEnd,
    ...
    QHash<TmpShape*, QSet<LzConnectPoint*>> usedPortsByShape;
    ...
    auto preferredPortForBinding = [&](TmpShape* shape, const QPointF& nodeAnchor, const CimTerminalBinding* binding) {
        if (!shape)
            return static_cast<LzConnectPoint*>(nullptr);

        if (auto* busbar = dynamic_cast<LzPowerBusbarSectionItem*>(shape))
        ...
        const QSet<LzConnectPoint*>& usedPorts = usedPortsByShape[shape];
        ...
        if (binding && orderedTransformerShape(shape))
        {
            ...
            if (preferredIndex >= 0)
            {
                LzConnectPoint* preferred = ports.value(preferredIndex, nullptr);
    ...
    auto ensureHiddenNodeHub = [&](const QString& nodeMrid, const QPointF& nodeAnchor) -> TmpShape* {
        ...
        auto* hub = new LzPowerBusbarSectionItem(QRectF(0.0, 0.0, 18.0, 4.0));
    ...
    auto connectShapes = [&](TmpShape* startShape, CONNECT_DIRECTION startDir,
        ...
        const QPointF effectiveAnchor = nodeAnchor.isNull()
            ...
            : nodeAnchor;
        LzConnectPoint* startPort = preferredPortForBinding(startShape, effectiveAnchor, startBinding);
        LzConnectPoint* endPort = preferredPortForBinding(endShape, effectiveAnchor, endBinding);
        if (!startPort)
        ...
        auto* line = new LzConnectLine(QRectF(0, 0, 1, 1));
        scene->addItem(line);
        line->setConnectStrategy(BROKEN_CONNECTION);
        line->setCornerStyle(LzConnectLine::Sharp);
    ...
    auto fallbackConnectByRows = [&]() {
        ...
        for (int i = 1; i < orderedShapes.size(); ++i)
        {
            ...
            const bool anchorIsBusbar = dynamic_cast<LzPowerBusbarSectionItem*>(anchor) != nullptr;
            const bool nextIsBusbar = dynamic_cast<LzPowerBusbarSectionItem*>(next) != nullptr;
    ...
    for (const QString& nodeMrid : orderedNodeMrids)
    {
        ...
        for (const CimTerminalBinding& binding : bindings)
        {
            ...
            if (dynamic_cast<LzPowerBusbarSectionItem*>(shape))
                busbarShape = shape;
            if (dynamic_cast<LzPowerJunctionItem*>(shape))
