#ifndef CIMDRAWCONNECTCONFIG_H
#define CIMDRAWCONNECTCONFIG_H
...
inline CONNECTION_STRATEGY cimdrawDefaultConnectLineStrategy()
{
    return BROKEN_CONNECTION;
}

/** 中间折点编辑是否约束为正交（仅 DIRECT 为自由斜向） */
inline bool cimdrawConnectStrategyUsesOrthogonalEdit(CONNECTION_STRATEGY strategy)
{
    return strategy != DIRECT_CONNECTION;
}

/** 图元拖动结束后倾向全量避障重算（非 lite L 形） */
inline bool cimdrawConnectStrategyPrefersFullRecompute(CONNECTION_STRATEGY strategy)
{
...
static constexpr double CimdrawConnectLineLayerZ = -10000.0;
/** 选中编辑时置于符号之上，便于拖点且不被选中框完全挡住 */
static constexpr double CimdrawConnectLineHighlightLayerZ = 1.0;

/** draw.io 风格：场景坐标下吸附到连接点圆心的最大距离（像素） */
static constexpr double CimdrawConnectPointSnapRadiusPx = 20.0;

/** 选择工具：按住 Ctrl 从图元本体/母线边找最近端口拉线；直接点中端口无需 Ctrl */
inline bool cimdrawWiringGestureActive(Qt::KeyboardModifiers mods)
{
    return (mods & Qt::ControlModifier) != 0;
}

/** 母线：光标距上下沿导体的最大吸附距离（像素），便于松手/拖线接到母线 */
static constexpr double CimdrawBusbarSnapRadiusPx = 48.0;

/** 正交路由时端口沿法向引出到图元外的距离，避免导线被符号本体遮挡 */
static constexpr double CimdrawWireRoutingLeadOutPx = 18.0;

/** 连线在视觉上与图元端口保持的最小留白，避免导线紧贴符号边缘 */
static constexpr double CimdrawWireEndpointVisualGapPx = 14.0;
