#include "LzSceneViewportController.h"
...
QRectF LzSceneViewportController::contentsRect(LzScene* scene) const
{
    if (!scene)
        return QRectF();
    ...
    {
        if (!item || !item->isVisible() || lzIsHandle(item))
            continue;
...
bool LzSceneViewportController::dispatchCompatMouseEvent(LzScene* scene,
