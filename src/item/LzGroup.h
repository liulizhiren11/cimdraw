...
struct LzConnectLinePathSnapshot
{
    LzConnectLine* line = nullptr;
    QVector<QPointF> scenePoints;
    ConnectorPathRoutingMode routingMode = ConnectorPathRoutingMode::Auto;
};

class LzGroup : public QObject, public TmpBase<QGraphicsItemGroup>
{
    Q_OBJECT
...
public:
    explicit LzGroup(QGraphicsItem* parent = nullptr);
    LzGroup(const QRectF& pos,QGraphicsItem* parent = nullptr);
    ...
    static void restoreConnectLineSnapshots(const QVector<LzConnectLinePathSnapshot>& snapshots);
    /** 分组/解组时按场景折点原样恢复，不做中间折点保形重排 */
    static void restoreConnectLineSnapshotsExact(const QVector<LzConnectLinePathSnapshot>& snapshots);
    /** 仅收集作为分组直接子项的连接线（不含组外相连线） */
    static void collectConnectLineSnapshotsInGroup(const LzGroup* group,
                                                 QVector<LzConnectLinePathSnapshot>& out);
    ...
    void copyItemFrameFrom(const LzGroup* source);
