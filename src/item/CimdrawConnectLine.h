#ifndef CIMDRAWCONNECTLINE_H
#define CIMDRAWCONNECTLINE_H
...
struct CimdrawConnectLineTopologyBindingSnapshot
{
    ...
    bool isRelationLink = false;

    bool hasEndpointBindings() const
    {
        return !startNodeStableId.isEmpty()
    ...
    {
        return isRelationLink || !edgeStableId.isEmpty() || hasEndpointBindings();
    }
};

class CimdrawConnectLine : public CimdrawItem
{
    Q_OBJECT
public:
    ...
    enum { Type = UserType + 4 };  // 必须与 CimdrawItem::Type(UserType+3) 区分，否则矩形等会被误当作连接线导致崩溃
    ...
    explicit CimdrawConnectLine(QGraphicsItem* parent = nullptr);

    CimdrawConnectLine(const QRectF& pos,QGraphicsItem* parent = nullptr);

    ~CimdrawConnectLine() override;
    ...
    */
    ...
    void setStartConnectPort(CimdrawConnectPoint* port);
    void setEndConnectPort(CimdrawConnectPoint* port);
    CimdrawConnectPoint* startConnectPort() const;
    CimdrawConnectPoint* endConnectPort() const;
    ...
    */
    ...
    /// 与 planOrthogonalPath 相同的障碍收集规则（叶子图元，不含 CimdrawGroup/连接点），供 Nudging 等复用
    static QList<QGraphicsItem*> collectRoutingObstacles(const CimdrawConnectLine* line, CimdrawScene* scene,
    ...
    void attachEndAndRecompute(QGraphicsItem* endItem, CimdrawConnectPoint* endPort);
    ...
    /**
     * @brief    显示连接线右键菜单（供 CimdrawHandle 等转发调用，避免访问 protected）
     */
    ...
    CimdrawConnectorAlgorithm::ConnectorRoutingStrategy routingStrategy() const;
    void setRoutingStrategy(CimdrawConnectorAlgorithm::ConnectorRoutingStrategy strategy);

    /// 路径优先模式：最短 / 少拐弯 / 平衡（proposal-phase1 任务 1.5）
    CimdrawConnectorAlgorithm::PathPriorityMode pathPriorityMode() const;
    void setPathPriorityMode(CimdrawConnectorAlgorithm::PathPriorityMode mode);

    QString resolvedTopologyEdgeStableId();
    TopologyEdgeMeta resolvedRelationEdgeMeta();
    TopologyEdgeLookupHint resolvedRelationEdgeLookupHint();
    void refreshResolvedTopologyBindings();
    void applyRelationEdgeMeta(const TopologyEdgeMeta& edgeMeta);
    TopologyEdgeMeta persistedRelationEdgeMeta() const;
    QString persistedEndpointBindingId(bool startEndpoint) const;
    bool restorePersistedEndpointItems(const QHash<QString, QGraphicsItem*>& topologyBindingIndex);
    void restorePersistedEndpointPorts(const QVector<QPointF>& scenePath = {});
    bool hasTopologyBindings() const;

    /** 两端均为拓扑节点图元时的语义连线（与几何连线工具共用 CimdrawConnectLine） */
...
private:
    ...
    void loadPersistedTopologyBindingSnapshot(const CimdrawConnectLineTopologyBindingSnapshot& snapshot);
    void syncTopologyEndpointsFromItems();
    void applyTopologyBindingSnapshot(const CimdrawConnectLineTopologyBindingSnapshot& snapshot);
    CimdrawConnectLineTopologyBindingSnapshot topologyBindingSnapshot() const;
    void refreshTopologyBindings(bool scheduleSceneRebuild = false);
    void handleEndpointItemDestroyed(bool startEndpoint);

    Q_DECLARE_PRIVATE(CimdrawConnectLine);
    QScopedPointer<CimdrawConnectLinePrivate> d_ptr;
};
