...
#include "CimdrawConnectionParameters.h"
...
bool cimdrawSnapTmpShapeSceneToRectEdge(const TmpBase<QGraphicsItem>* shape, const QPointF& scenePos, qreal maxDist,
                                   QPointF& anchorScene);

template <typename BaseType = QGraphicsItem>
class TmpBase : public BaseType, public ICimdrawTopologyParticipant
{
    friend CimdrawConnectPoint* cimdrawEnsureTmpShapeConnectPointAtScene(TmpBase<QGraphicsItem>* shape,
                                                               const QPointF& scenePos);
    friend void cimdrawRemoveTmpShapeDynamicConnectPointIfUnused(TmpBase<QGraphicsItem>* shape, CimdrawConnectPoint* port);
    friend void cimdrawPruneStaleConnectPointsOnShape(TmpBase<QGraphicsItem>* shape);
    friend bool cimdrawSnapTmpShapeSceneToRectEdge(const TmpBase<QGraphicsItem>* shape, const QPointF& scenePos,
...
public:
        ...
        :BaseType(parent)
    {
        ...
        {
            CimdrawHandle* handle = new CimdrawHandle(this, i,false);
    ...
    */
    virtual void updatePoints()
    {
        ...
        {
            CimdrawConnectPoint* point = item;
    ...
    */
	...
	{
        const QVector<CimdrawHandle*>::const_reverse_iterator hend = handleStruct.handles.rend();
        for (QVector<CimdrawHandle*>::const_reverse_iterator it = handleStruct.handles.rbegin(); it != hend; ++it)
		{
    ...
    */
     ...
     * @date     2025-04-30
    */
    void setDrawTool(CimdrawTool* tool)
    {
        tool = tool;
    }
    ...
    */
    ...
    QString cimdrawObjectId() const { return m_cimdrawObjectId; }
    void setCimdrawObjectId(const QString& id) { m_cimdrawObjectId = id; }
    QString ensureCimdrawObjectId()
    {
        if (m_cimdrawObjectId.isEmpty())
            m_cimdrawObjectId = CimdrawSnowflakeId::instance().nextIdString();
        return m_cimdrawObjectId;
    }
    void writeXmlObjectId(QDomElement* g)
    {
        if (!g)
            return;
        ensureCimdrawObjectId();
        g->setAttribute(QStringLiteral("cimdrawObjectId"), m_cimdrawObjectId);
    }
    void readXmlObjectId(const QDomElement& g)
    {
        if (g.hasAttribute(QStringLiteral("cimdrawObjectId")))
            m_cimdrawObjectId = g.attribute(QStringLiteral("cimdrawObjectId"));
    }

    /** 拓扑节点稳定 ID（一阶段与 cimdrawObjectId 共用存储；XML 另写 topologyNodeId 便于对齐方案文档） */
    QString topologyNodeStableId() const { return m_cimdrawObjectId; }
    void setTopologyNodeStableId(const QString& id) { m_cimdrawObjectId = id; }
    QString ensureTopologyNodeStableId() { return ensureCimdrawObjectId(); }
    ...
    */
    ...
    const QVector<CimdrawConnectPoint*>& wiringConnectPorts() const
    {
...
protected:
    ...
    //连接线
    ConnectType::CimdrawConnectionParameters connectStruct;
    //连接点
    ConnectPoint::ConnectPoint pointStruct;
    //是否允许选中内部子级
    bool selectChild = false;

    CimdrawTool* tool = nullptr;

    /** @see cimdrawObjectId() */
    QString m_cimdrawObjectId;
};
