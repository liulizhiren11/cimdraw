...
LzAStar::LzAStar() = default;

void LzAStar::setGridStep(int stepPixels)
{
    m_gridStep = qBound(MinGridStep, stepPixels, StepSize);
}

void LzAStar::setMaxSearchIterations(int maxIterations)
{
    m_maxSearchIterations = maxIterations;
}

QRectF LzAStar::expandedObstacleSceneRect(const QGraphicsItem* item, int margin)
{
    if (!item)
        return {};
    QRectF base;
...
static int proximityCost(const QPoint& pt, const QList<QGraphicsItem*>& items)
{
    ...
    for (QGraphicsItem* it : items)
    {
        if (!it || !it->isVisible())
            continue;
        const QRectF expanded = LzAStar::expandedObstacleSceneRect(it, ObstacleClearanceMargin);
        if (!expanded.isValid())
            continue;
...
bool LzAStar::isBlocked(const QPoint & pt,
...
QPoint LzAStar::nearestWalkable(const QPoint & goalAligned,
    QList<QGraphicsItem*> items,
    int maxRing)
{
...
bool LzAStar::segmentIntersectsObstacle(const QPoint& a, const QPoint& b,
    const QList<QGraphicsItem*>& items,
    const QPoint& goalAllow) const
{
    ...
    for (QGraphicsItem* it : items)
    {
        ...
        QRectF expanded = LzAStar::expandedObstacleSceneRect(it, ObstacleClearanceMargin);
        if (!expanded.isValid())
            continue;
...
int LzAStar::heuristic(const QPoint & a, const QPoint & b)
{
    return (qAbs(a.x() - b.x()) + qAbs(a.y() - b.y())) / m_gridStep;
}

// 获取当前点的邻居节点，排除掉障碍点（与 isBlocked 一致使用膨胀矩形，确保路径不穿过图元）
QList<QPoint> LzAStar::getNeighbors(const QPoint& p, const QPoint& goal, QList<QGraphicsItem*> items)
{
...
QList<QPoint> LzAStar::reconstructPath(const QHash<QPoint, QPoint>&cameFrom, QPoint current)
{
    QList<QPoint> totalPath;
...
// 将点对齐到网格
QPoint LzAStar::alignToGrid(const QPoint& pt, int step)
{
    return QPoint((pt.x() / step) * step, (pt.y() / step) * step);
}

// A* 算法，带有方向变化惩罚；turnPenalty 与 proximityScale 用于路径优先模式
QList<QPoint> LzAStar::aStarWithDirectionPenalty(const QPoint& start, const QPoint& goal, QList<QGraphicsItem*> items,
...
QList<QPoint> LzAStar::simplifyOrthogonalPath(const QList<QPoint>& path)
{
...
QList<QPoint> LzAStar::optimizePathForVisioStyle(const QList<QPoint>& path, const QPoint& start, const QPoint& end,
...
QList<QPoint> LzAStar::ensureOrthogonalPath(const QList<QPoint>& path, const QPoint& originalStart, const QPoint& originalEnd)
{
    if (path.isEmpty())
    {
...
QList<QPoint> LzAStar::simplifyToMinimalPath(const QList<QPoint>& path)
{
    if (path.size() <= 3)
    {
...
bool LzAStar::pathSegmentsIntersectObstacles(const QList<QPoint>& path, const QList<QGraphicsItem*>& items) const
{
...
bool LzAStar::segmentIntersectsObstacles(const QPoint& a, const QPoint& b, const QList<QGraphicsItem*>& items) const
{
...
static bool orthogonalConnectionClear(const QPoint& a, const QPoint& b,
    const QList<QGraphicsItem*>& items, const LzAStar* astar)
{
    if (a == b)
        return true;
    if (a.x() == b.x() || a.y() == b.y())
...
QList<QPoint> LzAStar::simplifyPathByStringPulling(const QList<QPoint>& path, const QList<QGraphicsItem*>& items) const
{
    if (path.size() <= 2)
        return path;
...
QList<QPoint> LzAStar::simplifyPathByDiagonalShortcuts(const QList<QPoint>& path, const QList<QGraphicsItem*>& items) const
{
    if (path.size() <= 2)
        return path;
