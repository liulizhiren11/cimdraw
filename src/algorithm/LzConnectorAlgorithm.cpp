#include "LzConnectorAlgorithm.h"
#include "LzAStar.h"
...
LzConnectorAlgorithm::InsertWaypointResult LzConnectorAlgorithm::projectPointOnPolyline(
...
QPointF LzConnectorAlgorithm::orthogonalCornerPosition(
    const QPointF& prev,
    const QPointF& next,
...
bool LzConnectorAlgorithm::preferHorizontalFirst(const QPointF& start, const QPointF& end)
{
...
static bool polylineSegmentsClear(const QList<QPoint>& path, LzAStar& astar,
                                                 ...
                                                 const QList<QGraphicsItem*>& obstacles, LzAStar& astar)
{
    ...
    else
    {
        ...
        if (LzConnectorAlgorithm::preferHorizontalFirst(startScene, endScene))
        {
...
static int countPathObstacleHits(const QList<QPoint>& path, LzAStar& astar,
                                 const QList<QGraphicsItem*>& obstacles)
{
    int hits = 0;
    for (int i = 0; i + 1 < path.size(); ++i)
...
static void pathPriorityParams(LzConnectorAlgorithm::PathPriorityMode priority,
                                int* outTurnPenalty, qreal* outProximityScale)
{
    ...
    switch (priority)
    {
    case LzConnectorAlgorithm::PathShortest:    turnPenalty = 2;  proximityScale = 0.5; break;
    case LzConnectorAlgorithm::PathFewerBends:  turnPenalty = 15; proximityScale = 1.0; break;
    case LzConnectorAlgorithm::PathBalanced:    turnPenalty = 5;  proximityScale = 1.0; break;
    }
    *outTurnPenalty = turnPenalty;
    *outProximityScale = proximityScale;
}

QVector<QPoint> LzConnectorAlgorithm::planOrthogonalPathQuick(
...
QVector<QPoint> LzConnectorAlgorithm::planOrthogonalPath(
    const QPointF& startScene,
    const QPointF& endScene,
    const QList<QGraphicsItem*>& obstacles,
    ConnectorRoutingStrategy strategy,
    LzConnectorAlgorithm::PathPriorityMode priority)
{
    ...
    for (int stepIdx = 0; stepIdx < effStepCount; ++stepIdx)
    {
        ...
        {
            path = LzVisibilityGraph::planPath(startScene, endScene, routeObstacles, &astar, turnPenalty, gridStep);
            if (path.size() < 2)
                path.clear();
        }
        if (path.isEmpty())
...
QList<QVector<QPointF>> LzConnectorAlgorithm::computeNudgingOffsets(const QList<QVector<QPointF>>& paths,
                                                                   qreal spacing,
                                                                   qreal minOverlap)
{
