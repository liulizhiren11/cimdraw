    ...
    LzCenterWidget* centerWidget) const
{
    ...
    context.afterCreate = [frame](LzView* view, LzScene* scene) {
        if (!frame || !view || !scene)
            return;
...
FrameDocumentController::OpenSceneContext FrameSceneLifecycleCoordinator::makeOpenSceneContext(LzFrame* frame) const
{
    ...
    QLineEdit* messageLineEdit) const
{
    ...
    context.onBrowserDockReady = [frame](CimModelBrowserDock* dock) {
        ...
        QObject::connect(dock, &CimModelBrowserDock::objectActivated,
                         frame, &LzFrame::onCimBrowserObjectActivated);
    };
    context.createTargetView = [frame]() { return frame ? frame->newPowerScene() : nullptr; };
    context.renameCurrentTab = [frame](const QString& title) {
        if (frame)
