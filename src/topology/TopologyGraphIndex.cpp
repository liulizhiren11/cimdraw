...
QString stableNodeIdForItem(QGraphicsItem* item)
{
    if (!item)
        return {};
...
void applyNodeHighlight(QGraphicsItem* item, bool on, const QColor& penC, const QColor& brushC)
{
    ...
    if (auto* li = dynamic_cast<LzItem*>(item))
        applyLzItem(li);
    else if (auto* grp = dynamic_cast<LzGroup*>(item)) {
        // LzGroup 无 LzItem 基类，但 TmpBase 提供相同样式接口；此处用 QObject 动态属性不可行，直接调 Group 的 pen/brush
        if (on) {
...
void TopologyGraphIndex::rebuildFromScene(QGraphicsScene* scene)
{
    ...
    for (QGraphicsItem* gi : items) {
        if (gi && gi->data(CimSceneBuilder::CimGeneratedVisualHelperDataKey).toBool())
            continue;
        if (dynamic_cast<LzConnectLine*>(gi))
            continue;
        const QString nid = stableNodeIdForItem(gi);
        if (!nid.isEmpty() && gi->parentItem() == nullptr)
            m_nodeIdToItem.insert(nid, gi);
    }

    for (QGraphicsItem* gi : items) {
        auto* line = dynamic_cast<LzConnectLine*>(gi);
        if (!line)
            continue;
        if (line->data(CimSceneBuilder::CimGeneratedVisualHelperDataKey).toBool())
            continue;
        line->syncTopologyEndpointsFromItems();
        const QString eid = line->topologyEdgeStableId();
        if (eid.isEmpty())
            continue;

        TopologyEdgeMeta meta;
        meta.edgeStableId = eid;
        meta.relationType = line->topologyRelationType();
        meta.startNodeStableId = line->topologyStartNodeStableId();
        meta.endNodeStableId = line->topologyEndNodeStableId();
        meta.startPortKey = line->topologyStartPortKey();
        meta.endPortKey = line->topologyEndPortKey();
...
LzConnectLine* TopologyGraphIndex::connectLineByEdgeId(const QString& edgeStableId) const
{
...
void TopologyGraphIndex::highlightEdges(const QStringList& edgeStableIds, bool highlighted, const QColor& strokeColor)
{
    ...
    for (const QString& eid : edgeStableIds) {
        LzConnectLine* ln = connectLineByEdgeId(eid);
        if (!ln)
            continue;
        if (highlighted) {
