...
QVector<CimdrawConnectPoint*> connectPointsForItem(QGraphicsItem* item)
{
    QVector<CimdrawConnectPoint*> ports;
    if (!item)
        return ports;
...
QString portKeyForIndex(const CimdrawConnectPoint* port, int index)
{
    if (!port)
...
QString fallbackPortTag(const CimdrawConnectPoint* port, int index)
{
...
QString portTokenForTopology(const CimdrawConnectPoint* port, int index)
{
...
QString portDisplayNameForTopology(const CimdrawConnectPoint* port, int index)
{
...
TmpShape* asTopologyShape(QGraphicsItem* item)
{
    if (!item || item->data(CimSceneBuilder::CimGeneratedVisualHelperDataKey).toBool()
        || cimdrawIsHandle(item) || item->type() == CimdrawConnectPoint::Type || item->type() == CimdrawConnectLine::Type)
        return nullptr;
    auto* shape = qgraphicsitem_cast<TmpShape*>(item);
    const auto* participant = cimdrawTopologyParticipantForItem(shape);
    if (!shape || !participant || !participant->participatesInTopology()
        || participant->topologyDomain() != CimdrawTopologyDomain::PowerSystem)
        return nullptr;
    return shape;
}

PowerTopologyRole roleForWiringItem(const CimdrawWiringItemBase* item)
{
    if (!item)
...
int effectiveDeviceSwitchPosition(const CimdrawWiringItemBase* item,
                                  ...
                                  int overrideSwitchPosition = -1)
{
    ...
    case PowerTopologyRole::GroundSwitch:
    {
        ...
        return item->effectiveSwitchPosition();
    }
    default:
        return 1;
    }
...
static PowerTopologyAnalysisSnapshot buildImpl(CimdrawScene* scene,
                                               ...
                                               int overrideSwitchPosition)
{
    ...
    QHash<const CimdrawConnectPoint*, QString> portToTerminalId;
    StringUnionFind uf;
    ...
    for (QGraphicsItem* item : scene->items(Qt::AscendingOrder))
    {
        ...
        const QVector<CimdrawConnectPoint*> ports = connectPointsForItem(shape);
        if (ports.isEmpty())
            continue;

        PowerTopologyDeviceRecord record;
        record.deviceId = cimdrawEnsureTopologyBindingIdForShape(shape);
        record.className = shape->className();
        record.displayName = displayNameForShape(shape);

        auto* wiringItem = dynamic_cast<CimdrawWiringItemBase*>(shape);
        const PowerTopologyRole role = roleForWiringItem(wiringItem);
        record.role = role;
        if (wiringItem)
        {
            record.flowSign = wiringItem->effectiveFlowSign();
        ...
        const int effectiveSwitchPosition = effectiveDeviceSwitchPosition(
            wiringItem, role, record.deviceId, overrideDeviceId, overrideSwitchPosition);

        for (int i = 0; i < ports.size(); ++i)
        {
            CimdrawConnectPoint* port = ports.at(i);
            if (!port)
                continue;
    ...
    for (QGraphicsItem* item : scene->items(Qt::AscendingOrder))
    {
        ...
        CimdrawConnectPoint* startPort = line->startConnectPort();
        CimdrawConnectPoint* endPort = line->endConnectPort();
        ...
        const QString wireId = line->resolvedTopologyEdgeStableId();
        if (wireId.isEmpty())
            continue;

        PowerTopologyConductorRecord conductor;
        conductor.wireId = wireId;
...
PowerTopologyAnalysisSnapshot PowerTopologyBuilder::build(CimdrawScene* scene)
{
    return buildImpl(scene, QString(), -1);
}

PowerTopologyAnalysisSnapshot PowerTopologyBuilder::buildWithSwitchOverride(CimdrawScene* scene,
