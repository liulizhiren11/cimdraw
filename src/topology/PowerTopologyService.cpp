...
TerminalGraph buildTerminalGraph(const PowerTopologyAnalysisSnapshot& snapshot, bool directed)
{
    ...
    for (const auto& device : snapshot.devices)
    {
        ...
        for (auto it = device.internalConnectivity.cbegin(); it != device.internalConnectivity.cend(); ++it)
        {
            ...
            if (directed && powerTopologyDeviceSupportsDirectedTerminalFlow(device))
            {
                const QString forwardFrom = powerTopologyDeviceForwardFromTerminalId(device);
                const QString forwardTo = powerTopologyDeviceForwardToTerminalId(device);
