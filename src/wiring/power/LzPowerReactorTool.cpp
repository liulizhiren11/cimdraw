...
LzPowerReactorTool::LzPowerReactorTool(QObject* parent)
    : LzTool(parent)
{
    name = tr("电抗器 L");
...
LzPowerReactorTool::~LzPowerReactorTool() = default;

LzDrawTypeId LzPowerReactorTool::getDrawType() const
{
    return LZ_WSYM_REACTOR;
}

QGraphicsItem* LzPowerReactorTool::createObject(LzScene* scene, QRectF* pos, bool addToScene)
{
    auto* item = new LzPowerReactorItem(*pos);
...
QIcon LzPowerReactorTool::getIcon(const QSize& size, const QVariant& para) const
{
    Q_UNUSED(para);
    QPixmap pm(size);
    ...
    const QSizeF def = LzPowerReactorItem::defaultSize();
    ...
    const QRectF iconRect(avail.center().x() - def.width() * scale * 0.5,
                          ...
                          def.width() * scale,
                          def.height() * scale);
    LzPowerReactorItem::drawSymbol(&painter, iconRect, LzWiringRunState::Normal, false);
    return QIcon(pm);
}


void LzPowerReactorTool::onMousePress(LzScene* scene, QGraphicsSceneMouseEvent* evt)
{
    ...
    const QPointF local = evt->scenePos();
        QRectF rc(local, LzPowerReactorItem::defaultSize());
