...
LzPowerArcCoilTool::LzPowerArcCoilTool(QObject* parent)
    : LzTool(parent)
{
    name = tr("消弧线圈");
...
LzPowerArcCoilTool::~LzPowerArcCoilTool() = default;

LzDrawTypeId LzPowerArcCoilTool::getDrawType() const
{
    return LZ_WSYM_ARC_COIL;
}

QGraphicsItem* LzPowerArcCoilTool::createObject(LzScene* scene, QRectF* pos, bool addToScene)
{
    auto* item = new LzPowerArcCoilItem(*pos);
...
QIcon LzPowerArcCoilTool::getIcon(const QSize& size, const QVariant& para) const
{
    Q_UNUSED(para);
    QPixmap pm(size);
    ...
    const QSizeF def = LzPowerArcCoilItem::defaultSize();
    ...
    const QRectF iconRect(avail.center().x() - def.width() * scale * 0.5,
                          ...
                          def.width() * scale,
                          def.height() * scale);
    LzPowerArcCoilItem::drawSymbol(&painter, iconRect, LzWiringRunState::Normal, false);
    return QIcon(pm);
}


void LzPowerArcCoilTool::onMousePress(LzScene* scene, QGraphicsSceneMouseEvent* evt)
{
    ...
    const QPointF local = evt->scenePos();
        QRectF rc(local, LzPowerArcCoilItem::defaultSize());
