...
CimdrawPowerCableTool::CimdrawPowerCableTool(QObject* parent)
    : CimdrawTool(parent)
{
    name = tr("电缆进线");
...
CimdrawPowerCableTool::~CimdrawPowerCableTool() = default;

CimdrawDrawTypeId CimdrawPowerCableTool::getDrawType() const
{
    return CIMDRAW_WSYM_CABLE;
}

QGraphicsItem* CimdrawPowerCableTool::createObject(CimdrawScene* scene, QRectF* pos, bool addToScene)
{
    auto* item = new CimdrawPowerCableItem(*pos);
...
QIcon CimdrawPowerCableTool::getIcon(const QSize& size, const QVariant& para) const
{
    Q_UNUSED(para);
    QPixmap pm(size);
    ...
    const QSizeF def = CimdrawPowerCableItem::defaultSize();
    ...
    const QRectF iconRect(avail.center().x() - def.width() * scale * 0.5,
                          ...
                          def.width() * scale,
                          def.height() * scale);
    CimdrawPowerCableItem::drawSymbol(
        &painter, iconRect, CimdrawWiringRunState::Normal, false, CimdrawWiringSymbolStyle::instance().standard());
    return QIcon(pm);
}


void CimdrawPowerCableTool::onMousePress(CimdrawScene* scene, QGraphicsSceneMouseEvent* evt)
{
    ...
    const QPointF local = evt->scenePos();
        QRectF rc(local, CimdrawPowerCableItem::defaultSize());
