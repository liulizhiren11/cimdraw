...
LzPowerSvgCompTool::LzPowerSvgCompTool(QObject* parent)
    : LzTool(parent)
{
    name = tr("静止无功 SVG");
...
LzPowerSvgCompTool::~LzPowerSvgCompTool() = default;

LzDrawTypeId LzPowerSvgCompTool::getDrawType() const
{
    return LZ_WSYM_SVG_COMP;
}

QGraphicsItem* LzPowerSvgCompTool::createObject(LzScene* scene, QRectF* pos, bool addToScene)
{
    auto* item = new LzPowerSvgCompItem(*pos);
...
QIcon LzPowerSvgCompTool::getIcon(const QSize& size, const QVariant& para) const
{
    Q_UNUSED(para);
    QPixmap pm(size);
    ...
    const QSizeF def = LzPowerSvgCompItem::defaultSize();
    ...
    const QRectF iconRect(avail.center().x() - def.width() * scale * 0.5,
                          ...
                          def.width() * scale,
                          def.height() * scale);
    LzPowerSvgCompItem::drawSymbol(&painter, iconRect, LzWiringRunState::Normal, false, 1);
    return QIcon(pm);
}


void LzPowerSvgCompTool::onMousePress(LzScene* scene, QGraphicsSceneMouseEvent* evt)
{
    ...
    const QPointF local = evt->scenePos();
        QRectF rc(local, LzPowerSvgCompItem::defaultSize());
