...
LzPowerArresterTool::LzPowerArresterTool(QObject* parent)
    : LzTool(parent)
{
    name = tr("避雷器 MOA");
...
LzPowerArresterTool::~LzPowerArresterTool() = default;

LzDrawTypeId LzPowerArresterTool::getDrawType() const
{
    return LZ_WSYM_ARRESTER;
}

QGraphicsItem* LzPowerArresterTool::createObject(LzScene* scene, QRectF* pos, bool addToScene)
{
    auto* item = new LzPowerArresterItem(*pos);
...
QIcon LzPowerArresterTool::getIcon(const QSize& size, const QVariant& para) const
{
    Q_UNUSED(para);
    QPixmap pm(size);
    ...
    const QSizeF def = LzPowerArresterItem::defaultSize();
    ...
    const QRectF iconRect(avail.center().x() - def.width() * scale * 0.5,
                          ...
                          def.width() * scale,
                          def.height() * scale);
    LzPowerArresterItem::drawSymbol(
        &painter, iconRect, LzWiringRunState::Normal, false, LzWiringSymbolStyle::instance().standard());
    return QIcon(pm);
}

void LzPowerArresterTool::onMousePress(LzScene* scene, QGraphicsSceneMouseEvent* evt)
{
    ...
    QRectF rc(local, LzPowerArresterItem::defaultSize());
