...
LzPowerGroundTool::LzPowerGroundTool(QObject* parent)
    : LzTool(parent)
{
    name = tr("接地");
...
LzPowerGroundTool::~LzPowerGroundTool() = default;

LzDrawTypeId LzPowerGroundTool::getDrawType() const
{
    return LZ_SLD_GROUND;
}

QGraphicsItem* LzPowerGroundTool::createObject(LzScene* scene, QRectF* pos, bool addToScene)
{
    auto* item = new LzPowerGroundItem(*pos);
...
QIcon LzPowerGroundTool::getIcon(const QSize& size, const QVariant& para) const
{
    Q_UNUSED(para);
    QPixmap pm(size);
    ...
    const QSizeF def = LzPowerGroundItem::defaultSize();
    ...
    const QRectF iconRect(avail.center().x() - def.width() * scale * 0.5,
                          ...
                          def.width() * scale,
                          def.height() * scale);
    LzPowerGroundItem::drawSymbol(
        &painter, iconRect, LzWiringRunState::Normal, false, LzWiringSymbolStyle::instance().standard());
    return QIcon(pm);
}


void LzPowerGroundTool::onMousePress(LzScene* scene, QGraphicsSceneMouseEvent* evt)
{
    ...
    const QPointF local = evt->scenePos();
        QRectF rc(local, LzPowerGroundItem::defaultSize());
