...
CimdrawPowerGroundTool::CimdrawPowerGroundTool(QObject* parent)
    : CimdrawTool(parent)
{
    name = tr("接地");
...
CimdrawPowerGroundTool::~CimdrawPowerGroundTool() = default;

CimdrawDrawTypeId CimdrawPowerGroundTool::getDrawType() const
{
    return CIMDRAW_SLD_GROUND;
}

QGraphicsItem* CimdrawPowerGroundTool::createObject(CimdrawScene* scene, QRectF* pos, bool addToScene)
{
    auto* item = new CimdrawPowerGroundItem(*pos);
...
QIcon CimdrawPowerGroundTool::getIcon(const QSize& size, const QVariant& para) const
{
    Q_UNUSED(para);
    QPixmap pm(size);
    ...
    const QSizeF def = CimdrawPowerGroundItem::defaultSize();
    ...
    const QRectF iconRect(avail.center().x() - def.width() * scale * 0.5,
                          ...
                          def.width() * scale,
                          def.height() * scale);
    CimdrawPowerGroundItem::drawSymbol(
        &painter, iconRect, CimdrawWiringRunState::Normal, false, CimdrawWiringSymbolStyle::instance().standard());
    return QIcon(pm);
}


void CimdrawPowerGroundTool::onMousePress(CimdrawScene* scene, QGraphicsSceneMouseEvent* evt)
{
    ...
    const QPointF local = evt->scenePos();
        QRectF rc(local, CimdrawPowerGroundItem::defaultSize());
