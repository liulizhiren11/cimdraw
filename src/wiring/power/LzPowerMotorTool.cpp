...
LzPowerMotorTool::LzPowerMotorTool(QObject* parent)
    : LzTool(parent)
{
    name = tr("电动机");
...
LzPowerMotorTool::~LzPowerMotorTool() = default;

LzDrawTypeId LzPowerMotorTool::getDrawType() const
{
    return LZ_SLD_MOTOR;
}

QGraphicsItem* LzPowerMotorTool::createObject(LzScene* scene, QRectF* pos, bool addToScene)
{
    auto* item = new LzPowerMotorItem(*pos);
...
QIcon LzPowerMotorTool::getIcon(const QSize& size, const QVariant& para) const
{
    Q_UNUSED(para);
    QPixmap pm(size);
    ...
    const QSizeF def = LzPowerMotorItem::defaultSize();
    ...
    const QRectF iconRect(avail.center().x() - def.width() * scale * 0.5,
                          ...
                          def.width() * scale,
                          def.height() * scale);
    LzPowerMotorItem::drawSymbol(&painter, iconRect, LzWiringRunState::Normal, false);
    return QIcon(pm);
}

void LzPowerMotorTool::onMousePress(LzScene* scene, QGraphicsSceneMouseEvent* evt)
{
    ...
    QRectF rc(local, LzPowerMotorItem::defaultSize());
