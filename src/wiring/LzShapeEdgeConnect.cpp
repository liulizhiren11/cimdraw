...
bool isLiveConnectPointOnShape(const LzConnectPoint* port, const QGraphicsItem* shape)
{
    return port && port->scene() && port->parentItem() == shape;
}

bool isDynamicWiringPort(const LzConnectPoint* port)
{
...
void lzPruneStaleConnectPointsOnShape(TmpBase<QGraphicsItem>* shape)
{
    ...
    QVector<LzConnectPoint*> livePoints;
    livePoints.reserve(shape->childItems().size());
    for (QGraphicsItem* child : shape->childItems())
    {
        LzConnectPoint* point = qgraphicsitem_cast<LzConnectPoint*>(child);
                                   ...
                                   QPointF& anchorScene)
{
    if (!shape || maxDist <= 0.0)
        return false;
    if (dynamic_cast<const LzPowerBusbarSectionItem*>(shape))
        return false;
...
LzConnectPoint* lzEnsureTmpShapeConnectPointAtScene(TmpBase<QGraphicsItem>* shape, const QPointF& scenePos)
{
    ...
    lzPruneStaleConnectPointsOnShape(shape);

    QPointF anchorScene;
    if (!lzSnapTmpShapeSceneToRectEdge(shape, scenePos, LzConnectPointSnapRadiusPx, anchorScene))
        return nullptr;
    ...
    for (LzConnectPoint* p : shape->wiringConnectPorts())
    {
    ...
    auto* point = new LzConnectPoint(shape);
...
void lzCleanupAfterConnectLineRemoved(LzConnectLine* line)
{
    ...
    LzConnectPoint* const startPort = line->startConnectPort();
    LzConnectPoint* const endPort = line->endConnectPort();
    ...
    const auto prunePort = [line](QGraphicsItem* item, LzConnectPoint* port) {
        if (!port || !item)
            return;
        ...
        {
            lzRemoveTmpShapeDynamicConnectPointIfUnused(shape, port);
            if (line->startConnectPort() == port)
                line->setStartConnectPort(nullptr);
            if (line->endConnectPort() == port)
                line->setEndConnectPort(nullptr);
